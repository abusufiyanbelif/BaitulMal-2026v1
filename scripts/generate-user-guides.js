...
function inferUseCases(route, content) {
    const cases = [];
    if (route.includes('dashboard')) cases.push('Monitor organization statistics and metrics in real-time.');
    ...
    if (route.includes('beneficiaries')) cases.push('Maintain records of individuals receiving organization support.');
    ...
    if (content.includes('upload')) cases.push('Attach supporting documentation or evidence to organization records.');
    if (content.includes('verify')) cases.push('Perform administrative verification of submitted data.');

    if (cases.length === 0) cases.push('Standard organization operations and data management.');
...
## 🎯 Purpose
${page.purpose || 'Organization module for administrative operations.'}
