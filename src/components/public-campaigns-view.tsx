  ...
  return (
    <div className="space-y-8">
       <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-4 bg-primary/5 p-4 rounded-xl border border-primary/20">
              <div className="w-full sm:w-auto flex-1 min-w-[200px]">
                <Input placeholder="Search Campaigns..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full h-9 text-xs border-primary/20 focus-visible:ring-primary text-primary font-normal" disabled={isLoading}/>
              </div>
              <div className="grid grid-cols-2 sm:flex gap-2 w-full sm:w-auto">
                <Select value={statusFilter} onValueChange={setStatusFilter} disabled={isLoading}><SelectTrigger className="w-full sm:w-[130px] h-9 text-xs font-normal border-primary/20 text-primary capitalize"><SelectValue placeholder="Status" /></SelectTrigger><SelectContent className="rounded-[12px] shadow-dropdown"><SelectItem value="All" className="font-normal capitalize">All Statuses</SelectItem><SelectItem value="Active" className="font-normal capitalize">Active</SelectItem><SelectItem value="Completed" className="font-normal capitalize">Completed</SelectItem><SelectItem value="Upcoming" className="font-normal capitalize">Upcoming</SelectItem></SelectContent></Select>
                <Select value={categoryFilter} onValueChange={setCategoryFilter} disabled={isLoading}><SelectTrigger className="w-full sm:w-[130px] h-9 text-xs font-normal border-primary/20 text-primary capitalize"><SelectValue placeholder="Category" /></SelectTrigger><SelectContent className="rounded-[12px] shadow-dropdown"><SelectItem value="All" className="font-normal capitalize">All Categories</SelectItem><SelectItem value="Ration" className="font-normal capitalize">Ration</SelectItem><SelectItem value="Relief" className="font-normal capitalize">Relief</SelectItem><SelectItem value="General" className="font-normal capitalize">General</SelectItem></SelectContent></Select>
              </div>
              <div className="flex items-center gap-2 w-full sm:w-auto sm:border-l sm:border-primary/10 sm:pl-3">
                  <Select value={selectedYear} onValueChange={(val) => { setSelectedYear(val); setDateRange(undefined); }} disabled={isLoading}><SelectTrigger className="flex-1 sm:w-[100px] h-9 text-xs text-primary font-normal"><SelectValue placeholder="Year" /></SelectTrigger><SelectContent className="rounded-[12px] shadow-dropdown"><SelectItem value="All" className="font-normal">Year</SelectItem>{availableYears.map(y => <SelectItem key={y} value={y} className="font-normal">{y}</SelectItem>)}</SelectContent></Select>
                  <Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className={cn("h-9 px-3 text-xs font-normal border-primary/20 flex-1 sm:flex-none", !dateRange ? "text-muted-foreground" : "text-primary")} disabled={isLoading}><CalendarIcon className="mr-2 h-3 w-3" /> Range</Button></PopoverTrigger><PopoverContent sideOffset={12} className="w-auto p-0 rounded-[16px] shadow-dropdown border-primary/10" align="center"><Calendar initialFocus mode="range" selected={dateRange} onSelect={(d) => { setDateRange(d); if (d?.from) { setSelectedYear('All'); } }} numberOfMonths={1} /></PopoverContent></Popover>
                  {(selectedYear !== 'All' || dateRange) && <Button variant="ghost" size="icon" className="h-8 w-8 text-primary shrink-0" onClick={() => { setSelectedYear('All'); setDateRange(undefined); }}><X className="h-4 w-4" /></Button>}
              </div>
        </div>
      </div>
