        ...
        >
          ...
          <CardHeader className="p-4">
            <div className="flex justify-between items-start gap-2">
                <CardTitle className="w-full break-words text-sm sm:text-base font-black line-clamp-2 uppercase tracking-tight text-[#138808]">
                    {campaign.campaignNumber && <span className="text-[#138808]">#{campaign.campaignNumber} </span>}{campaign.name}
                </CardTitle>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="h-8 w-8 flex-shrink-0 text-[#138808]" onClick={(e) => e.stopPropagation()}>
                            <MoreHorizontal className="h-4 w-4" />
                        </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenuItem onClick={() => router.push(`/campaign-members/${campaign.id}/summary`)} className="cursor-pointer font-bold text-[#138808]">
                        ...
                        {canUpdate && (
                            <>
                                <DropdownMenuSub>
                                    <DropdownMenuSubTrigger className="font-bold text-[#138808]"><span>Change Status</span></DropdownMenuSubTrigger>
                                    <DropdownMenuPortal>
                                        <DropdownMenuSubContent>
                                ...
                                <DropdownMenuSub>
                                    <DropdownMenuSubTrigger className="font-bold text-[#138808]"><span>Verification</span></DropdownMenuSubTrigger>
                                    <DropdownMenuPortal>
                                        <DropdownMenuSubContent>
                        ...
                        <DropdownMenuSeparator />
                        {canCreate && (
                            <DropdownMenuItem onClick={() => handleCopyClick(campaign)} className="cursor-pointer font-bold text-[#138808]">
            ...
            <CardDescription className="text-[10px] uppercase font-black tracking-widest text-muted-foreground">{campaign.startDate} to {campaign.endDate}</CardDescription>
          </CardHeader>
          <CardContent className="flex-grow space-y-3 p-4 pt-0">
            ...
            {(campaign.targetAmount || 0) > 0 && (
                <div className="space-y-1.5">
                    ...
                    <p className="text-center text-[10px] text-muted-foreground font-mono font-bold">Goal: ₹{(campaign.targetAmount || 0).toLocaleString('en-IN')}</p>
                </div>
            )}
          </CardContent>
          <CardFooter className="p-2 border-t bg-primary/5">
            <Button asChild className="w-full text-xs font-black uppercase tracking-widest hover:bg-[#138808] hover:text-white text-[#138808]" size="sm" variant="ghost">
...
export default function CampaignPage() {
  ...
  if (!isLoading && userProfile && !canViewCampaigns) {
    return (
      <main className="container mx-auto p-4 md:p-8 text-[#138808]">
        <div className="mb-4"><Button variant="outline" asChild className="font-bold border-primary/20 text-[#138808]"><Link href="/"><ArrowLeft className="mr-2 h-4 w-4" /> Back to Home</Link></Button></div>
    ...
    <>
      <main className="container mx-auto p-4 sm:p-6 space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <Button variant="outline" asChild size="sm" className="font-bold border-primary/20 text-[#138808] hover:bg-[#138808]/10">
            <Link href="/dashboard"><ArrowLeft className="mr-2 h-4 w-4" /> Dashboard</Link>
          </Button>
          {canCreate && !isLoading && (
            <Button asChild size="sm" className="bg-[#16a34a] hover:bg-[#16a34a]/90 text-white font-bold">
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-black tracking-tighter uppercase text-[#138808]">Campaign Hub</h1>
        ...
        <Card className="animate-fade-in-zoom shadow-md border-primary/10 bg-white">
          <CardHeader className="p-4 sm:p-6 border-b bg-primary/5">
            <div className="flex flex-wrap items-center gap-3">
                <Input placeholder="Search initiatives..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="max-w-xs h-9 text-xs border-primary/20 focus-visible:ring-[#138808] text-[#138808] font-bold" disabled={isLoading}/>
                <Select value={statusFilter} onValueChange={setStatusFilter} disabled={isLoading}><SelectTrigger className="w-[130px] h-9 text-xs font-bold border-primary/20 text-[#138808]"><SelectValue placeholder="Status" /></SelectTrigger><SelectContent><SelectItem value="All">All Statuses</SelectItem><SelectItem value="Active">Active</SelectItem><SelectItem value="Completed">Completed</SelectItem><SelectItem value="Upcoming">Upcoming</SelectItem></SelectContent></Select>
                <Select value={categoryFilter} onValueChange={setCategoryFilter} disabled={isLoading}><SelectTrigger className="w-[130px] h-9 text-xs font-bold border-primary/20 text-[#138808]"><SelectValue placeholder="Category" /></SelectTrigger><SelectContent><SelectItem value="All">All Categories</SelectItem><SelectItem value="Ration">Ration</SelectItem><SelectItem value="Relief">Relief</SelectItem><SelectItem value="General">General</SelectItem></SelectContent></Select>
                <div className="flex items-center gap-2 border-l border-primary/10 pl-3 ml-1">
                    <Select value={selectedYear} onValueChange={(val) => { setSelectedYear(val); setDateRange(undefined); }} disabled={isLoading}><SelectTrigger className="w-[100px] h-9 text-xs font-bold border-primary/20 text-[#138808]"><SelectValue placeholder="Year" /></SelectTrigger><SelectContent><SelectItem value="All">Year</SelectItem>{availableYears.map(y => <SelectItem key={y} value={y}>{y}</SelectItem>)}</SelectContent></Select>
                    <Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className={cn("h-9 px-3 text-xs font-bold border-primary/20", !dateRange ? "text-muted-foreground" : "text-[#138808]")} disabled={isLoading}><CalendarIcon className="mr-2 h-3 w-3" /> Range</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="end"><Calendar initialFocus mode="range" selected={dateRange} onSelect={(d) => { setDateRange(d); if (d?.from) { setSelectedYear('All'); } }} numberOfMonths={2} /></PopoverContent></Popover>
          ...
          <CardContent className="p-4 sm:p-6 bg-card/30">
            {isLoading ? (
                  ...
                  <AccordionItem key={section.id} value={section.id} className="border-primary/10 rounded-xl px-4 bg-white shadow-sm overflow-hidden">
                    <AccordionTrigger className="hover:no-underline py-5 group">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-lg font-black tracking-tight uppercase text-[#138808]">{section.title}</span>
                        <Badge variant="secondary" className="rounded-full h-5 text-[10px] font-black bg-primary/10 text-[#138808]">{section.items.length}</Badge>
                      </div>
                    </AccordionTrigger>
              ...
              <div className="text-center py-24 bg-primary/5 rounded-2xl border-2 border-dashed border-primary/20">
                  <HandHelping className="h-16 w-16 mx-auto text-[#138808]/20 mb-4" />
                  <p className="text-[#138808]/60 font-black uppercase tracking-widest text-sm">No initiatives found matching filters.</p>
              </div>
            )}
          </CardContent>
        </Card>
