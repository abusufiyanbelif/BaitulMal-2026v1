...
export default function BeneficiariesPage() {
  ...
  if (!lead) return <p className="text-center mt-20 text-[#138808] font-black uppercase">Lead not found.</p>;

  return (
    <main className="container mx-auto p-4 md:p-8 space-y-6">
        <div className="mb-4"><Button variant="outline" asChild className="font-bold border-primary/20 text-[#138808]"><Link href="/leads-members"><ArrowLeft className="mr-2 h-4 w-4" /> Back to Hub</Link></Button></div>
        <h1 className="text-3xl font-black tracking-tighter text-[#138808] uppercase">{lead.name}</h1>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <h2 className="text-2xl font-black text-[#138808] uppercase tracking-tighter">Lead Beneficiaries ({beneficiaries?.length || 0})</h2>
                                    ...
                                    <AccordionItem key={b.id} value={b.id} className="border-b border-primary/5 last:border-0 hover:bg-primary/[0.02] transition-colors">
                                        ...
                                        <AccordionContent className="bg-primary/[0.01] px-4 pt-0 pb-4 border-t border-primary/5">
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 px-12">
                                                ...
                                                <div className="space-y-1">
                                                    <p className="text-[10px] font-black uppercase text-[#138808]/60 tracking-wider">Requirement Details</p>
                                                    <p className="text-sm font-bold text-[#1B5E20]">Estimated: ₹{(b.kitAmount || 0).toLocaleString()} | Allocated: ₹{(b.zakatAllocation || 0).toLocaleString()}</p>
                                                </div>
                                                <div className="space-y-1">
                                                    <p className="text-[10px] font-black uppercase text-[#138808]/60 tracking-wider">Category Assigned</p>
                                                    <Badge variant="secondary" className="text-[10px] font-black uppercase">{b.itemCategoryName || 'General Support'}</Badge>
                                                </div>
                                            </div>
                                        </AccordionContent>
                                    </AccordionItem>
