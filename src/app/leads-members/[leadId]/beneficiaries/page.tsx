...
const StatCard = ({ title, count, description, icon: Icon, colorClass }: { title: string, count: number, description: string, icon: any, colorClass?: string }) => (
    <Card className="flex-1 min-w-[150px] interactive-hover border-primary/10 bg-white">
        <CardContent className="p-4 flex items-start justify-between">
            <div className="space-y-1">
                <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{title}</p>
                <p className="text-2xl font-black text-primary">{count}</p>
                <p className="text-[10px] text-muted-foreground whitespace-nowrap font-bold">{description}</p>
            </div>
...
export default function BeneficiariesPage() {
  ...
  if (!lead) return <p className="text-center mt-20 text-primary font-bold uppercase">Lead not found.</p>;

  return (
    <main className="container mx-auto p-4 md:p-8 space-y-6">
        <div className="mb-4"><Button variant="outline" asChild className="interactive-hover font-bold uppercase border-primary/20 text-primary"><Link href="/leads-members"><ArrowLeft className="mr-2 h-4 w-4" /> Back to Hub</Link></Button></div>
        <h1 className="text-3xl font-black tracking-tighter text-primary uppercase">{lead.name}</h1>
        ...
        <Card className="animate-fade-in-zoom shadow-md border-primary/10 bg-white">
            <CardHeader className="pb-4">
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                    <div className="space-y-1">
                        ...
                        <CardDescription className="font-bold text-[#138808]/70">Paginated results grouped by requirement categories.</CardDescription>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        ...
                        <Button size="sm" onClick={() => setIsFormOpen(true)} className="gap-2 font-black uppercase tracking-widest interactive-hover shadow-lg bg-primary hover:bg-primary/90 text-white"><PlusCircle className="h-4 w-4"/> Add New</Button>
                    </div>
                </div>
            </CardHeader>
            <CardContent className="space-y-6">
                ...
                <div className="border border-primary/10 rounded-lg overflow-hidden bg-card min-w-[900px]">
                                ...
                                <AccordionContent className="p-0">
                                    {list.map((b, idx) => (
                                        <div key={b.id} className={cn("items-center py-3 px-4 border-b border-primary/5 last:border-0 hover:bg-primary/5 transition-colors text-sm", gridClass)}>
                                            ...
                                            <div className="truncate"><div className="font-black text-[#138808] truncate">{b.name}</div><div className="text-[10px] text-muted-foreground font-mono">{b.phone || 'N/A'}</div></div>
                                            ...
                                            <div className="text-right text-[10px] font-bold uppercase truncate pl-2 text-[#138808]">{b.referralBy}</div>
                    ...
                    {filteredBeneficiaries.length === 0 && <div className="text-center py-20 text-primary/40 font-black uppercase tracking-widest bg-primary/5">No beneficiaries found.</div>}
                </div>
            </CardContent>
            {totalPages > 1 && (
                <CardFooter className="flex items-center justify-between border-t border-primary/10 py-4 bg-primary/5">
                    <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">Page {currentPage} of {totalPages}</p>
