  ...
  const filteredAndSortedDonations = useMemo(() => {
    ...
    if (categoryFilter.length > 0) {
        items = items.filter(d => {
            ...
            return d.type ? categoryFilter.includes(d.type) : false;
        });
    }
            ...
            <CardContent className="p-0">
                <ScrollArea className="w-full">
                    <div className={cn("bg-[hsl(var(--table-header-bg))] border-b border-primary/10 text-[11px] font-bold text-[hsl(var(--table-header-fg))] tracking-tight", donationGridClass)}>
                        ...
                        <SortableHeader sortKey="amountForThisLead" className="text-right" sortConfig={sortConfig} handleSort={handleSort}><Calculator className="mr-1 h-3 w-3 opacity-40"/> Amount</SortableHeader>
                                ...
                                <React.Fragment key={donation.id}>
                                    <div className={cn("hover:bg-[hsl(var(--table-row-hover))] transition-colors cursor-pointer border-b border-primary/10 bg-white items-center", donationGridClass)} onClick={() => setOpenRows(prev => ({...prev, [donation.id]: !prev[donation.id]}))}>
                                        ...
                                        <div className="text-right font-bold font-mono text-primary text-sm flex items-center justify-end gap-1">
                                            ...
                                            ₹{donation.amountForThisLead.toFixed(2)}
                                        </div>
