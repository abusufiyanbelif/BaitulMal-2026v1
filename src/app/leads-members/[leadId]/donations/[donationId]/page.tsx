    ...
    if (!donation || !lead) {
        return (
            <main className="container mx-auto p-4 md:p-8 text-center font-normal">
                <p className="text-lg text-primary font-bold opacity-60">Donation Or Lead Record Not Found.</p>
                <Button asChild className="mt-4 font-bold border-primary/20 text-primary transition-transform active:scale-95" variant="outline"><Link href="/leads-members"><ArrowLeft className="mr-2 h-4 w-4" /> Back To Leads</Link></Button>
            </main>
        );
    }
    ...
    return (
        <main className="container mx-auto p-4 md:p-8 space-y-6 text-primary font-normal animate-fade-in-up">
            ...
            <div className="space-y-6">
                {!donation.donorId && (
                    <div className="p-4 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl flex items-center gap-3 animate-fade-in-down shadow-sm">
                        <AlertCircle className="h-5 w-5 text-amber-600 shrink-0" />
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold">Identity Mapping Required</p>
                            <p className="text-xs font-normal opacity-80">This donation is currently an unlinked "dummy" record. Please map it to a verified Donor Profile.</p>
                        </div>
                        <Button variant="link" asChild className="font-bold text-amber-800 p-0 h-auto underline shrink-0">
                        ...
                        <Card className="border-primary/10 shadow-sm bg-white overflow-hidden">
                            ...
                            <CardContent className="space-y-6 pt-6">
                                ...
                                {donation.suggestions && (
                                    <div className="space-y-1"><p className="text-[10px] font-bold text-muted-foreground capitalize tracking-widest">Staff Recommendations</p><p className="text-sm font-normal bg-primary/[0.02] p-4 rounded-lg italic border border-primary/5 leading-relaxed">"{donation.suggestions}"</p></div>
                                )}
                            </CardContent>
                        </Card>
                    )}
            ...
            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
                <DialogContent className="max-w-2xl h-[90vh] flex flex-col p-0 overflow-hidden rounded-[16px] border-primary/10">
                    ...
                    <div className="flex-1 overflow-hidden relative">
                        <DonationForm donation={donation} onSubmit={handleFormSubmit} onCancel={() => setIsFormOpen(false)} campaigns={allCampaigns || []} leads={allLeads || []} defaultLinkId={`lead_${leadId}`} />
                    </div>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={isImageViewerOpen} onOpenChange={setIsImageViewerOpen}>
                <DialogContent className="max-w-4xl max-h-[95vh] flex flex-col p-0 rounded-[12px] border-primary/10 overflow-hidden shadow-2xl animate-fade-in-zoom">
                    <DialogHeader className="px-6 py-4 bg-primary/5 border-b">
                        <DialogTitle className="text-xl font-bold text-primary tracking-tight capitalize tracking-widest">{imageToView ? imageToView.title : 'Evidence Viewer'}</DialogTitle>
                    </DialogHeader>
