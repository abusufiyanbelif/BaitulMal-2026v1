        ...
        >
          ...
          <CardHeader className="p-4">
            <div className="flex justify-between items-start gap-2">
                <CardTitle className="w-full break-words text-sm sm:text-base font-black line-clamp-2 tracking-tight text-primary">{lead.name}</CardTitle>
                  <DropdownMenu>
            ...
            <CardDescription className="text-[10px] font-bold tracking-normal text-muted-foreground">{lead.startDate} to {lead.endDate}</CardDescription>
        </CardHeader>
        <CardContent className="flex-grow space-y-3 p-4 pt-0">
              <div className="flex justify-between items-center text-xs">
                <Badge variant="secondary" className="text-[10px] font-black">{lead.purpose}</Badge>
                <Badge 
         ...
         <CardFooter className="p-2 border-t bg-primary/5">
            <Button asChild className="w-full text-xs font-black tracking-tight text-primary hover:bg-primary hover:text-white" size="sm" variant="ghost">
    ...
    <>
      <main className="container mx-auto p-4 sm:p-6 space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <Button variant="outline" asChild size="sm" className="interactive-hover font-bold border-primary/20 text-primary hover:bg-primary/10"><Link href="/dashboard"><ArrowLeft className="mr-2 h-4 w-4" /> Dashboard</Link></Button>
          {canCreate && !isLoading && <Button asChild size="sm" className="font-black tracking-tight interactive-hover shadow-lg bg-primary hover:bg-primary/90 text-white"><Link href="/leads-members/create"><Plus className="mr-2 h-4 w-4" /> New appeal</Link></Button>}
        </div>

        <div className="space-y-2">
          <h1 className="text-3xl font-black tracking-tight text-primary">Leads hub</h1>
          ...
          <CardHeader className="p-4 border-b bg-primary/5">
            <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-2 border-l border-primary/10 pl-3 ml-1">
                    ...
                    <Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className={cn("h-9 px-3 text-xs font-black border-primary/20", !dateRange ? "text-muted-foreground" : "text-primary")} disabled={isLoading}><CalendarIcon className="mr-2 h-3 w-3" /> Range</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="end"><Calendar initialFocus mode="range" selected={dateRange} onSelect={(d) => { setDateRange(d); if (d?.from) { setSelectedYear('All'); } }} numberOfMonths={2} /></PopoverContent></Popover>
                  ...
                  <AccordionItem key={section.id} value={section.id} className="border-primary/10 rounded-xl px-4 bg-white shadow-sm overflow-hidden">
                    <AccordionTrigger className="hover:no-underline py-5 group">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-lg font-black tracking-tight text-primary">{section.title}</span>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle className="font-black text-destructive">Delete appeal?</AlertDialogTitle><AlertDialogDescription className="font-bold text-primary/80">Permanently erase all data for '{leadToDelete?.name}'? This action cannot be undone.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel className="font-bold text-primary">Cancel</AlertDialogCancel><AlertDialogAction onClick={handleDeleteConfirm} className="bg-destructive text-white font-black hover:bg-destructive/90">Confirm deletion</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
      </AlertDialog>
