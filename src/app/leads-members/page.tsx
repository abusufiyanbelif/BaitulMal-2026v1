        ...
        >
          ...
          <CardHeader className="p-4">
            <div className="flex justify-between items-start gap-2">
                <CardTitle className="w-full break-words text-sm sm:text-base font-black line-clamp-2 uppercase tracking-tight text-[#138808]">{lead.name}</CardTitle>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenuItem onClick={() => router.push(`/leads-members/${lead.id}/summary`)} className="cursor-pointer font-bold text-[#138808]">
                        ...
                        {canCreate && (
                            <DropdownMenuItem onClick={() => handleCopyClick(lead)} className="cursor-pointer font-bold text-[#138808]">
            ...
            <CardDescription className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{lead.startDate} to {lead.endDate}</CardDescription>
        </CardHeader>
        <CardContent className="flex-grow space-y-3 p-4 pt-0">
              ...
              {(lead.targetAmount || 0) > 0 && (
                <div className="space-y-1.5">
                    ...
                    <p className="text-center text-[10px] text-muted-foreground font-mono font-bold">Goal: ₹{(lead.targetAmount || 0).toLocaleString('en-IN')}</p>
                </div>
            )}
        </CardContent>
         <CardFooter className="p-2 border-t bg-primary/5">
            <Button asChild className="w-full text-xs font-black uppercase tracking-widest text-[#138808] hover:bg-[#138808] hover:text-white" size="sm" variant="ghost">
  ...
  if (!isLoading && userProfile && !canViewLeads) {
    return (
      <main className="container mx-auto p-4 md:p-8">
        <div className="mb-4"><Button variant="outline" asChild className="text-[#138808] border-primary/20"><Link href="/"><ArrowLeft className="mr-2 h-4 w-4" /> Back to Home</Link></Button></div>
    ...
    <>
      <main className="container mx-auto p-4 sm:p-6 space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <Button variant="outline" asChild size="sm" className="interactive-hover font-bold uppercase border-primary/20 text-[#138808]"><Link href="/dashboard"><ArrowLeft className="mr-2 h-4 w-4" /> Dashboard</Link></Button>
          {canCreate && !isLoading && <Button asChild size="sm" className="font-black uppercase tracking-widest interactive-hover shadow-lg bg-[#16a34a] hover:bg-[#16a34a]/90 text-white"><Link href="/leads-members/create"><Plus className="mr-2 h-4 w-4" /> New Appeal</Link></Button>}
        </div>

        <div className="space-y-2">
          <h1 className="text-3xl font-black tracking-tighter uppercase text-[#138808]">Leads Hub</h1>
        ...
        <Card className="animate-fade-in-zoom shadow-md border-primary/5 bg-white">
          <CardHeader className="p-4 border-b bg-primary/5">
            <div className="flex flex-wrap items-center gap-3">
                <Input placeholder="Search appeals..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="max-w-xs h-9 text-xs border-primary/20 focus-visible:ring-[#138808] text-[#138808] font-bold" disabled={isLoading}/>
                <Select value={statusFilter} onValueChange={setStatusFilter} disabled={isLoading}><SelectTrigger className="w-[130px] h-9 text-xs font-black uppercase border-primary/20 text-[#138808]"><SelectValue placeholder="Status" /></SelectTrigger><SelectContent><SelectItem value="All">All Statuses</SelectItem><SelectItem value="Active">Active</SelectItem><SelectItem value="Completed">Completed</SelectItem><SelectItem value="Upcoming">Upcoming</SelectItem></SelectContent></Select>
                <Select value={purposeFilter} onValueChange={setPurposeFilter} disabled={isLoading}><SelectTrigger className="w-[180px] h-9 text-xs font-bold uppercase border-primary/20 text-[#138808]"><SelectValue placeholder="Purpose" /></SelectTrigger><SelectContent><SelectItem value="All">All Purposes</SelectItem>{[...new Set((leadsWithProgress || []).map(l => l.purpose))].map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select>
          ...
          <CardContent className="p-4 sm:p-6 bg-card/30">
            {isLoading ? (
              ...
              <div className="text-center py-24 bg-primary/5 rounded-2xl border-2 border-dashed border-primary/20">
                  <Lightbulb className="h-16 w-16 mx-auto text-[#138808]/20 mb-4" />
