...
import { Settings, Save, Loader2, CheckSquare, Edit, X, RefreshCw, DatabaseZap, ShieldCheck, Bell, Smartphone, KeyRound } from 'lucide-react';
        ...
        <Card className="animate-fade-in-up border-primary/10">
         ...
         <Card className="animate-fade-in-up border-red-200 bg-red-50/30 shadow-none overflow-hidden mt-6">
             <CardHeader className="bg-red-50 border-b border-red-100 pb-4">
                 ...
                 <CardDescription className="text-xs font-normal text-red-700/70">Global session management and emergency access termination for the beneficiary portal.</CardDescription>
             </CardHeader>
             <CardContent className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-6">
                 <div className="space-y-1 text-center sm:text-left">
                     <p className="text-sm font-bold text-red-900">Terminate Active Beneficiary Sessions</p>
                     <p className="text-[10px] text-red-700/60 font-medium max-w-sm">
                         This action will immediately invalidate all active login sessions for every beneficiary profile. 
                 ...
                 <Button 
                     ...
                     onClick={async () => {
                         if (confirm("CRITICAL ACTION: Are you sure you want to log out ALL beneficiaries? This action will force immediate re-authentication across the entire system.")) {
                             setIsSubmitting(true);
                             const res = await revokeAllSessionsForRoleAction('Beneficiary');
                             setIsSubmitting(false);
