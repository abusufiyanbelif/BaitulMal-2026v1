  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
            ...
            <Settings /> Beneficiary Summary Visibility Settings
            </CardTitle>
            <CardDescription>
                    ...
                    <div className="space-y-3">
                        {VISIBILITY_OPTIONS.map(opt => (
                            <div key={`public_ben_${opt.id}`} className="flex items-center space-x-2">
                                ...
                                />
                                <Label htmlFor={`public_ben_${opt.id}`} className="cursor-pointer">{opt.name}</Label>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="space-y-3">
                        {VISIBILITY_OPTIONS.map(opt => (
                            <div key={`member_ben_${opt.id}`} className="flex items-center space-x-2">
                                ...
                                />
                                <Label htmlFor={`member_ben_${opt.id}`} className="cursor-pointer">{opt.name}</Label>
                            </div>
                        ))}
                    </div>
                </div>
