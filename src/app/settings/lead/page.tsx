...
export default function LeadSettingsPage() {
  ...
  const docRef = useMemoFirebase(() => (firestore) ? doc(firestore, 'settings', 'lead_visibility') : null, [firestore]);
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
            ...
            <Settings /> Lead Visibility Settings
            </CardTitle>
            <CardDescription>
                    ...
                    <div className="space-y-3">
                        {VISIBILITY_OPTIONS.map(opt => (
                            <div key={`public_${opt.id}`} className="flex items-center space-x-2">
                                ...
                                />
                                <Label htmlFor={`public_lead_${opt.id}`} className="cursor-pointer">{opt.name}</Label>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="space-y-3">
                        {VISIBILITY_OPTIONS.map(opt => (
                            <div key={`member_${opt.id}`} className="flex items-center space-x-2">
                                ...
                                />
                                <Label htmlFor={`member_lead_${opt.id}`} className="cursor-pointer">{opt.name}</Label>
                            </div>
                        ))}
                    </div>
                </div>
