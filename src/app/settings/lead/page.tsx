                        ...
                        <div className="space-y-3">
                            {VISIBILITY_OPTIONS.map(opt => (
                                <div key={`public_lead_${opt.id}`} className="flex items-center space-x-2">
                                    ...
                                    />
                                    <Label htmlFor={`public_lead_${opt.id}`} className="cursor-pointer font-normal text-[#4D805F]">{opt.name}</Label>
                                </div>
                            ))}
                        </div>
                    </div>
                        ...
                        <div className="space-y-3">
                            {VISIBILITY_OPTIONS.map(opt => (
                                <div key={`member_lead_${opt.id}`} className="flex items-center space-x-2">
                                    ...
                                    />
                                    <Label htmlFor={`member_lead_${opt.id}`} className="cursor-pointer font-normal text-[#4D805F]">{opt.name}</Label>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {MANDATORY_FIELDS.map(field => (
                        <div key={field.id} className="flex items-center space-x-2">
                            ...
                            />
                            <Label htmlFor={`mandatory_lead_${field.id}`} className="cursor-pointer font-normal text-[#4D805F]">{field.name}</Label>
                        </div>
                    ))}
                </div>
            </CardContent>
