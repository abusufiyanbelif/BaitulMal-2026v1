                                      ...
                                      <DropdownMenu>
                                          ...
                                          <DropdownMenuContent align="end">
                                              {linkedInitiatives.length === 0 ? (
                                                  ...
                                                  <DropdownMenuItem onClick={(e) => { e.stopPropagation(); router.push(`/${linkedInitiatives[0].linkType === 'campaign' ? 'campaign' : 'leads'}-members/${linkedInitiatives[0].linkId}/donations/${donation.id}`); }}>
                                                      <Eye className="mr-2 h-4 w-4" /> View in "{linkedInitiatives[0].linkName}"
                                                  </DropdownMenuItem>
                                              ) : (
                                                  <DropdownMenuSub>
                                                      <DropdownMenuSubTrigger><Eye className="mr-2 h-4 w-4" />View In...</DropdownMenuSubTrigger>
                                                      <DropdownMenuPortal><DropdownMenuSubContent>
                                                          {linkedInitiatives.map(link => (
                                                              <DropdownMenuItem key={link.linkId} onClick={(e) => { e.stopPropagation(); router.push(`/${link.linkType === 'campaign' ? 'campaign' : 'leads'}-members/${link.linkId}/donations/${donation.id}`); }}>
                                                                  {link.linkName}
                                                              </DropdownMenuItem>
                                                          ))}
