...
-   **When to Run**: Run this script **once** after a feature update introduces numbering, to backfill numbers for all your old records.
...
-   **When to Run**: Only if you have data from a previous version of the application that used the old `rationLists` object instead of the `itemCategories` array. If you started with a fresh project, you will likely never need this.
...
-   **What it Does**:
    ...
    -   It compares what it finds to the application's expected structure and prints a helpful report. It does not make any changes to your data or files.
