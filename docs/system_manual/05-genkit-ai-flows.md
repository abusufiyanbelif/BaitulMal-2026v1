...
-   [Overview of Genkit](#overview-of-genkit)
-   [Anatomy of an AI Flow](#anatomy-of-an-ai-flow)
-   [Creating a New AI Flow](#creating-a-new-ai-flow)
-   [Troubleshooting AI Errors](#troubleshooting-ai-errors)
...
-   **"API key not valid"**: Your `GEMINI_API_KEY` is missing, incorrect, or has restrictions. Verify it in your `.env` file and in [Google AI Studio](https://aistudio.google.com/app/apikey).
-   **"Permission denied" / "API not enabled"**: The "Generative Language API" is not enabled for your Google Cloud project. Enable it in the [Google Cloud Console](https://console.cloud.google.com/).
-   **"Model not found"**: The model name in your flow (e.g., `googleai/gemini-1.5-flash-latest`) is incorrect or not available. Check the [Google AI documentation](https://ai.google.dev/models/gemini) for the latest names.
